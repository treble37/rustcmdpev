...
mod tests {
    ...
    fn test2_process_explain_with_one_nested_plan() {
        let input = r#"[{"Plan":{"Alias":"c0","Node Type":"Seq Scan","Parallel Aware":false,"Plan Rows":50,"Plan Width":1572,"Relation Name":"coaches","Startup Cost":0.0,"Total Cost":10.5, "Plans":[{"Alias":"c1", "Actual Cost":500.0, "Actual Duration":300.0, "Actual Rows":200, "Node Type":"Seq Scan","Parallel Aware":false,"Plan Rows":50,"Plan Width":1572,"Relation Name":"coaches","Startup Cost":0.0,"Total Cost":5.5}]}}]"#;
        let explains: Vec<Explain> = serde_json::from_str(input).unwrap();
        let mut explain: Explain = explains.into_iter().nth(0).unwrap();
        explain = rustcmdpev::process_all(explain);
        println!("explain plan {:?}", explain.plan);
        assert_eq!(explain.total_cost, 16.0);
    ...
    #[test]
    fn test3_explain_with_one_join() {
        let input = r#"
            [
              {
                "Plan": {
